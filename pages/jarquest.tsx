...
const JarQuest = () => {
  ...
  useInterval(() => {
    ...
    if (shopOpenForBusiness && basicJars > 0) {
      ...
      if (Math.random() > 0.9) {
        setDeluxeJars(basicJars - 1);
        setMoney(money + 10);
      }
    }
  }, 100);

  const canMakeBasicJar = () => {
    return sandKgs >= 10 && inventory.includes(furnace);
  };
  const makeBasicJar = () => {
    if (canMakeBasicJar()) {
  ...
  const canMakeDeluxeJar = () => {
    return sandKgs >= 10 && inventory.includes(furnace) && learnedAboutJars;
  };
  const makeDeluxeJar = () => {
    if (canMakeBasicJar()) {
      setSandKgs(sandKgs - 10);
