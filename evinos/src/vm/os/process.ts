import { ProcStatus, File, Err } from "./constants";
import OS from "./os";
...
const configSyscalls = (process) => {
  ...
  return Object.keys(syscalls)
    ...
    .reduce((acc, { key, cb }) => Object.assign(acc, { [key]: cb }), {});
};
...
class Process {
  ...
  env: object;
  args: Array<string>;
  fds: Array<File>; // Should be actual files.
